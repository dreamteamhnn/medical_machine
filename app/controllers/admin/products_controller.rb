class Admin::ProductsController < Admin::BaseController
  ...
  def get_products_by_categories
    ...
    return @search.result.where('name LIKE ?', "%#{params[:term]}%").order('no_order IS NULL, no_order ASC, id ASC').page(params[:page]).per(params[:limit]) unless category_arr.size > 0
    Product.by_categories(category_arr).ransack(params[:q]).result.order('no_order IS NULL, no_order ASC, id ASC').page(params[:page]).per(params[:limit])
  end
end
